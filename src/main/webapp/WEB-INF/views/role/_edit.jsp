		...
		<form method="post" class="form-horizontal col-md-10 col-md-offset-1 "
			...
			<ul class="list-group">
							...
							<select class="form-control selectpicker  dropup"
								...
								<c:forEach items="${rlevels }" var="level">
									<option value="${level.key}"
										<c:if test="${rleveldesc==level.value }">selected="selected" </c:if>>${level.value}</option>
				...
				<li class="list-group-item">
					<div align="center">
						<div class="btn-group">
							<input type="reset" class="btn btn-danger btn-lg" value="重置"> <input type="submit" class="btn btn-warning btn-lg" id="submit"
								value="确认无误，注册">
						</div>
					</div>
				</li>
